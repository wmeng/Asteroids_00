...
void Asteroids::SetWindow(CoreWindow^ window)
{
	...
	window->PointerReleased +=
		ref new TypedEventHandler<CoreWindow^, PointerEventArgs^>(this, &Asteroids::OnPointerReleased);

    CCEGLView* eglView = new CCEGLView();
	eglView->Create(window);
    eglView->setViewName("Asteroids");
}
...
void Asteroids::OnPointerReleased(CoreWindow^ sender, PointerEventArgs^ args)
{
...
{
	HardwareButtons::BackPressed += ref new EventHandler<BackPressedEventArgs^>(this, &Asteroids::OnBackButtonPressed);   
	CoreWindow::GetForCurrentThread()->Activate();
}

void Asteroids::OnBackButtonPressed(Object^ sender, BackPressedEventArgs^ args)
{
